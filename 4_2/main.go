...
func main() {
	...
	for {
		...
		hasWinner, winningScores := numberPicker.findWinningBoards()
		if hasWinner {
			fmt.Printf("Found winners with number %v. Winning scores: %v\n", numberPicker.currentNumber, winningScores)
		}
	}

	for _, board := range numberPicker.boards {
